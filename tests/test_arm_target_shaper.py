...
class ArmTargetShaperTest(unittest.TestCase):
    def test_default_limits_match_the_feedforward_clamp_not_the_revoked_cap(self):
        shaper = ArmTargetShaper()
        self.assertEqual(shaper.velocity_limit, 6.0)
        self.assertEqual(shaper.accel_limit, 40.0)
    ...
    def test_a_fast_burst_is_accel_limited_at_the_production_defaults(self):
        shaper = ArmTargetShaper(dof=ARMS)
        shaper.shape(np.zeros(ARMS), now=0.0)
        # 15 rad/s for 25 ms would be a 0.375 rad step; accel 40 only allows 0.025 rad.
        shaped = shaper.shape(np.full(ARMS, 15.0 * 0.025), now=0.025)
        self.assertAlmostEqual(float(np.max(np.abs(shaped))), 40.0 * 0.025 * 0.025, places=9)
    ...
    def test_stats_track_how_often_the_target_outran_the_limit(self):
        shaper = ArmTargetShaper(velocity_limit=1.0, accel_limit=1e9, dof=ARMS)
        shaper.shape(np.zeros(ARMS), now=0.0)
        shaper.shape(np.full(ARMS, 0.001), now=0.01)
        shaper.shape(np.full(ARMS, 2.0), now=0.02)
...
class ArmControllerShaperTest(unittest.TestCase):
    """The controller must publish a ramped reference, and dq must follow it."""
    ...
    def test_hold_zeros_shaper_velocity(self):
        self.submit(0.0)
        self.now += 0.025
        self.submit(1.0)
        self.assertGreater(float(np.max(np.abs(self.controller.target_shaper.velocity))), 0.0)
